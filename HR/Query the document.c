...
char**** get_document(char* text) {
    ...
    // Initialize first paragraph and first sentence
    doc[p_idx] = malloc(MAX_CHARACTERS * sizeof(char**));
    doc[p_idx][s_idx] = malloc(MAX_CHARACTERS * sizeof(char*));
    ...
    while (1) {
        if (*cur == ' ' || *cur == '.' || *cur == '\n' || *cur == '\0') {
            // Word boundary detected: copy word
            int len = cur - start;
            if (len > 0) {
                doc[p_idx][s_idx][w_idx] = malloc(len + 1);
                memcpy(doc[p_idx][s_idx][w_idx], start, len);
                doc[p_idx][s_idx][w_idx][len] = '\0';
                w_idx++;
            }
            start = cur + 1; 
